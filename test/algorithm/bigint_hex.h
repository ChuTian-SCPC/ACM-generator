...
protected:
    ...
    //{hexm_e}
    BigIntBaseTest transbase(int32_t out_base) const {
        if (size() <= 8) {
            BigIntBaseTest sum(out_base);
            BigIntBaseTest base(out_base);
            BigIntBaseTest ownbase(out_base);
            ownbase.set(COMPRESS_MOD);
            BigIntBaseTest mul(out_base), t(out_base);
            base.set(1);
            t.set(v[0]);
            mul.raw_mul(base, t);
            sum.raw_add(mul);
            for (size_t i = 1; i < v.size(); i++) {
                base.raw_mul(ownbase, BigIntBaseTest(base));
                t.set(v[i]);
                mul.raw_mul(base, t);
                sum.raw_add(mul);
            }
            ...
            static std::vector<BigIntBaseTest> pow_list;
            ...
            for (; s < size() / 3; s *= 2, ++id) {
                if (s >= (size_t)1 << pow_list_cnt) {
                    pow_list.push_back(BigIntBaseTest(out_base));
            ...
            l.v.resize(s);
            BigIntBaseTest r = h.transbase(out_base);
            BigIntBaseTest sum(out_base);
    ...
    std::string out_mul(int32_t out_base = 10, int32_t pack = 0) const {
        BigIntBaseTest sum = transbase(out_base);
        std::string out;
        int32_t d = 0;
        for (size_t i = 0, j = 0;;) {
            if (j < 1) {
